...
# Load the CSV file
file_path = r"c:\Projects\smart-store-branton\data\raw\products_data.csv"
...
def handle_missing_values(df: pd.DataFrame) -> pd.DataFrame:
    ...
    # df['ProductID'].fillna('Unknown Product', inplace=True)
    ...
    # df['UnitPrice'].fillna(df['price'].median(), inplace=True)
    # df['category'].fillna(df['category'].mode()[0], inplace=True)
...
def remove_outliers(df: pd.DataFrame) -> pd.DataFrame:
    """
    Remove outliers based on thresholds using the IQR method.
    ...
    if 'quantity' in df.columns:  # Use lowercase 'quantity' to match cleaned column names
        # Convert quantity to numeric if necessary
        ...
        if df['quantity'].dtype in ['int64', 'float64']:
            ...
            logger.info(f"Quantity IQR: Q1={Q1}, Q3={Q3}, IQR={IQR}")
            logger.info(f"Quantity bounds: lower={lower_bound}, upper={upper_bound}")

            # Filter out outliers
            df = df[(df['quantity'] >= lower_bound) & (df['quantity'] <= upper_bound)]
            logger.info(f"Applied outlier removal to quantity: bounds [{lower_bound}, {upper_bound}]")
        else:
            logger.warning("Skipping outlier removal for quantity as it is not numeric.")
    else:
        logger.warning("Skipping outlier removal for quantity as the column is missing.")

    # Log the number of rows removed
    removed_count = initial_count - len(df)
    logger.info(f"Total outlier rows removed: {removed_count}")
